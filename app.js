...
function processData(sheetData, sheetName) {
    ...
    if (allWeeks.length === 0) {
        document.getElementById('content').innerHTML = '<p>표시할 주(week) 데이터가 없습니다.</p>';
        hideLoading();
        return;
    }
...
function renderCurrentWeek() {
    ...
    const weekGridData = { rowData: week.rows };

    const html = formatHandler.createFormattedTable(weekGridData, [], {}, null);
